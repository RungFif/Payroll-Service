                            ...
                            @foreach ($requests as $request)
                                ...
                                <tr x-data="{ open: false }">
                                    <td colspan="6" class="bg-gray-50 px-8 py-3">
                                        <button type="button" class="text-blue-600 hover:underline text-xs mb-2"
                                            @click="open = !open">
                                        ...
                                        <div x-show="open" x-transition>
                                            ...
                                            <div class="space-y-2 mb-2">
                                                @forelse($request->comments->take(3) as $comment)
                                                    ...
                                                    @endphp
                                                    <div
                                                        class="{{ $isAdmin ? 'bg-yellow-50 border-yellow-400' : 'bg-white' }} rounded p-2 border flex items-start">
                                                        @if ($isAdmin)
                                                            <svg class="h-4 w-4 text-yellow-500 mr-2 mt-0.5"
                                                                fill="currentColor" viewBox="0 0 20 20">
                                                                <path
                                                                    d="M10 2a2 2 0 012 2v1.586l.707.707A2 2 0 0114 7v2.586l.707.707A2 2 0 0116 12v2a2 2 0 01-2 2h-1v1a2 2 0 01-2 2 2 2 0 01-2-2v-1H6a2 2 0 01-2-2v-2a2 2 0 01.293-1.293L5 9.586V7a2 2 0 01.293-1.293L6 4.586V4a2 2 0 012-2h2z" />
                                                            </svg>
                                                        @endif
                                                        <div class="flex-1">
                                                            <div class="flex items-center mb-1">
                                                                <span
                                                                    class="font-semibold text-xs {{ $isAdmin ? 'text-yellow-700' : '' }}">
                                                                    {{ $comment->user->name }}
                                                                    @if ($isAdmin)
                                                                        <span
                                                                            class="ml-1 text-[10px] font-bold uppercase">(Admin)</span>
                                                                    @endif
                                                                </span>
                                                                <span
                                                                    class="ml-2 text-xs text-gray-400">{{ $comment->created_at->diffForHumans() }}</span>
                                                            </div>
                                                            @if (request('edit_comment') == $comment->id)
                                                                <form action="{{ route('comments.update', $comment) }}"
                                                                    ...
                                                                    <div class="flex space-x-2 mt-1">
                                                                        ...
                                                                        <a href="{{ url()->current() }}"
                                                                            class="px-2 py-1 bg-gray-200 rounded text-xs">Cancel</a>
                                                                    </div>
                                                                </form>
                                                            @else
                                                                <div class="text-gray-700 text-xs">{{ $comment->body }}
                                                                </div>
                                                            @endif
                                            ...
                                            <form action="{{ route('comments.store') }}" method="POST"
                                                ...
                                                <input type="hidden" name="commentable_type"
                                                    value="App\Models\EmployeeRequest">
